...
import  blogData  from "@/components/Sections/Benefit/benefits"; // assuming this is a function that fetches data
        ...
        <div className="list-blog">
          <div className="list grid xl:grid-cols-3 md:grid-cols-2 gap-8">
            {currentBlogs.map((item) => (
              <BlogItem key={item.id} data={item} type={'style-solution'} />
            ))}
          </div>
          {pageCount > 1 && (
